...
int main(int argc,char **argv)
{
    ...
    // extract the height and width  of the image from header
    int width, height;
    memcpy(&width, info + 18, sizeof(int));
    memcpy(&height, info + 22, sizeof(int));

    int window_size = strtol(argv[2],NULL,10);
    printf("The Window size: %d\n",window_size);
    printf("Dimensions of Image: (%d, %d)\n",width,height);
        
    int size = 3 * width * abs(height);

    printf("convert the bitmap to char array...\n");
    ...
    //convert the bitmap to char array
    ...
    {
        printf("Allocatating device memory on host.....\n");
